...
export default function VisualTeachingLab({
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-emerald-300/80 text-sm">
              Final register state shown below • PC: 0x{currentState.pc.toString(16).toUpperCase().padStart(8, '0')}
            </p>
          </div>
        </div>
      </motion.div>
        ...
        <div className="p-4">
          {currentState.programOutput && currentState.programOutput.trim() ? (
            ...
            <div className="font-mono text-sm text-slate-500 bg-black/50 rounded-lg p-4 italic">
              No output. Use syscall 1 (print int) or syscall 4 (print string) to print values.
            </div>
          )}
        </div>
      </motion.div>
          ...
          <div className="bg-slate-800/80 rounded-xl border border-cyan-500/30 overflow-hidden">
            ...
            <div className="p-5 space-y-6">
              {Object.entries(REGISTER_GROUPS).map(([groupName, regNames]) => {
                const groupRegs = regNames.filter(name => registers[name] !== undefined);
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-2">
                      {groupRegs.map((regName, idx) => {
                        ...
                        return (
                          <motion.button
                            key={regName}
                            onClick={() => setSelectedRegister(isSelected ? null : regName)}
                          ...
                          >
                            <div className={`text-xs font-mono font-semibold mb-1 ${
                              isNonZero ? 'text-emerald-400' : 'text-slate-500'
                            }`}>
                              {regName}
                            </div>
        ...
        <div className="space-y-4">
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="text-2xl font-mono font-bold text-white">{selectedRegister}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">Decimal</div>
                  <div className="text-lg font-mono text-emerald-400">{registers[selectedRegister]}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">Hexadecimal</div>
                  <div className="text-lg font-mono text-cyan-400">
                    0x{registers[selectedRegister].toString(16).toUpperCase().padStart(8, '0')}
                  </div>
                </div>
                <div>
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-emerald-400">{nonZeroRegisters.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Return Value ($v0)</span>
                <span className="font-mono text-white">{registers['$v0'] ?? 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Stack Pointer ($sp)</span>
                <span className="font-mono text-white">
                  {registers['$sp'] ? `0x${registers['$sp'].toString(16).toUpperCase()}` : 'N/A'}
                </span>
              </div>
            </div>
          </motion.div>
          ...
          >
            <p className="text-xs text-blue-300/80 leading-relaxed">
              💡 <strong>Tip:</strong> Click on any register to see its value in different formats. 
              Green highlighted registers have non-zero values after execution.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      {currentState.instructionAnalysis && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3 }}
        >
          <InstructionAnalysisPanel analysis={currentState.instructionAnalysis} />
        </motion.div>
      )}
    </div>
  );
