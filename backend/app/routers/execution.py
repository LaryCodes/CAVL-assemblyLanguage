...
from app.services.trace_parser import get_trace_parser, TraceParser
from app.services.state_manager import get_state_manager, StateManager
from app.services.mips_analyzer import get_mips_analyzer, MipsAnalysisError
...
async def execute_code(request: ExecuteRequest) -> ExecuteResponse:
    ...
    if not request.code or not request.code.strip():
        return ExecuteResponse(
            success=False,
            error="MIPS code cannot be empty"
        )
...
async def step_execution() -> StepResponse:
    ...
    if not state_manager.has_trace():
        return StepResponse(
            success=False,
            error="No program loaded. Call /api/execute first."
        )
    ...
    if new_state is None:
        return StepResponse(
            success=False,
            error="Failed to advance execution state"
        )
...
async def reset_execution() -> ResetResponse:
    ...
    if initial_state is None:
        return ResetResponse(
            success=False,
            error="Failed to reset execution state"
        )
