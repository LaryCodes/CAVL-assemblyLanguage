...
async def allocate_memory(request: AllocateRequest) -> AllocateResponse:
    ...
    if request.size <= 0:
        return AllocateResponse(
            success=False, error="Invalid allocation size: must be greater than 0"
        )
    ...
    try:
        ...
        if not result.success:
            return AllocateResponse(
                success=False, error=f"MARS execution failed: {result.error}"
            )
        ...
        if status != 0 or allocated_address < 0:
            return AllocateResponse(
                success=False, error="Allocation failed: no suitable free block found"
            )

        # Step 4: Parse heap state from memory dump
        heap_state = await _get_heap_state_from_execution(executor, parser, asm_code)

        # Step 5: Return result
        return AllocateResponse(
            success=True, address=allocated_address, heap=heap_state
        )

    except FileNotFoundError as e:
        return AllocateResponse(
            success=False, error=f"MIPS template not found: {str(e)}"
        )
    except Exception as e:
        return AllocateResponse(success=False, error=f"Unexpected error: {str(e)}")
...
async def free_memory(request: FreeRequest) -> FreeResponse:
    ...
    if request.address < 0:
        return FreeResponse(
            success=False, error="Invalid address: must be non-negative"
        )
    ...
    try:
        ...
        if status != 0:
            return FreeResponse(success=False, error="Free operation failed")

        # Step 4: Parse heap state from memory dump
        heap_state = await _get_heap_state_from_execution(executor, parser, asm_code)

        return FreeResponse(success=True, heap=heap_state)

    except FileNotFoundError as e:
        return FreeResponse(success=False, error=f"MIPS template not found: {str(e)}")
    except Exception as e:
        return FreeResponse(success=False, error=f"Unexpected error: {str(e)}")


@router.post("/init")
async def init_heap() -> dict[str, Any]:
    """
    ...
    try:
        ...
        if not result.success:
            return {"success": False, "error": f"MARS execution failed: {result.error}"}
...
async def _get_heap_state_from_execution(
    ...
    # Parse .data memory
    data_blocks = parser.parse_memory_dump(data_result.stdout, parser.DATA_START)
    ...
    heap_blocks = [b for b in data_blocks if heap_start <= b.address < heap_end]
...
@router.get("/state")
async def get_heap_state() -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": True, "heap": heap_state.model_dump()}
