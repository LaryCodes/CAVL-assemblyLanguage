...
class InstructionAnalysis:
    ...
    register_usage: Dict[str, int] = None  # Per-register usage
...
class MipsInstructionAnalyzer:
    ...
    ANALYZER_ASM = "instruction_analyzer.asm"
    
    # Magic number that MIPS writes to verify execution
    ANALYSIS_MAGIC = 0xCAFE1234
    ...
    def __init__(self, mars_jar_path: Optional[str] = None):
        """
        ...
        # Find MARS jar
        if mars_jar_path:
            self.mars_jar = Path(mars_jar_path)
        else:
        ...
        self.mips_core_dir = Path(__file__).parent.parent.parent.parent / "mips" / "core"
        
        # CRITICAL: Verify required MIPS file exists
        self.analyzer_path = self.mips_core_dir / self.ANALYZER_ASM
        ...
        if not self.analyzer_path.exists():
            raise MipsCoreMissingError(
                ...
                f"This file contains the instruction analysis logic implemented in MIPS assembly."
            )
        
        # Load the analyzer template
        self._analyzer_template = self._load_analyzer_template()
    ...
    def _assemble_to_words(self, user_code: str) -> List[int]:
        ...
        try:
            ...
            cmd = [
                "java", "-jar", str(self.mars_jar),
                "nc",  # No copyright
                "a",   # Assemble only (don't run)
                "dump", ".text", "HexText", dump_file,
                user_file
            ]
    ...
    def _inject_instructions(self, instructions: List[int]) -> str:
        ...
        modified = re.sub(
            r'instruction_buffer:\s*\.space\s+\d+',
            buffer_data,
            modified
        )
    ...
    def _parse_analysis_output(self, stdout: str, memory_dump: str, num_instructions: int) -> InstructionAnalysis:
        """
        ...
        if memory_dump:
            ...
            if len(values) > RESULTS_OFFSET + 9:
                ...
                analysis.i_type_count = values[idx + 1] if values[idx + 1] < 10000 else 0
                ...
                analysis.branch_count = values[idx + 4] if values[idx + 4] < 10000 else 0
                analysis.jump_count = values[idx + 5] if values[idx + 5] < 10000 else 0
                analysis.syscall_count = values[idx + 6] if values[idx + 6] < 10000 else 0
                analysis.other_count = values[idx + 7] if values[idx + 7] < 10000 else 0
                analysis.total_analyzed = values[idx + 8] if values[idx + 8] < 10000 else 0
    ...
    def analyze(self, user_code: str) -> InstructionAnalysis:
        ...
        try:
            ...
            analysis = self._parse_analysis_output(result.stdout, memory_dump, len(instructions))
            ...
            raise MipsAnalysisError("MIPS analysis timed out")
        except Exception as e:
...
# Singleton instance
_analyzer: Optional[MipsInstructionAnalyzer] = None
