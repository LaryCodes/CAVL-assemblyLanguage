...
class HeapState(BaseModel):
    ...
    fragmentation: float = Field(ge=0, le=100, default=0.0, description="Fragmentation percentage")
...
class MemoryBlock(BaseModel):
    ...
    size: int = Field(ge=0, default=4, description="Size in bytes (default word-aligned)")
    value: Optional[int] = Field(default=None, description="Memory content value")
    meta: Optional[HeapMeta] = Field(default=None, description="Heap metadata (for heap blocks only)")
    label: Optional[str] = Field(default=None, description="Optional label for the memory location")
...
class MemorySegment(BaseModel):
    ...
    blocks: List[MemoryBlock] = Field(default_factory=list, description="Memory blocks in this segment")
...
class InstructionAnalysis(BaseModel):
    ...
    load_count: int = Field(default=0, description="Memory load instructions (lw, lb, etc.)")
    store_count: int = Field(default=0, description="Memory store instructions (sw, sb, etc.)")
    branch_count: int = Field(default=0, description="Branch instructions (beq, bne, etc.)")
    ...
    register_usage: Dict[str, int] = Field(default_factory=dict, description="Per-register usage count")
    analysis_valid: bool = Field(default=False, description="Whether MIPS analysis completed successfully")
...
class ExecutionState(BaseModel):
    ...
    changed_registers: List[str] = Field(default_factory=list, description="Registers changed in last step")
    memory: MemoryState = Field(description="Current memory state")
    heap: HeapState = Field(description="Current heap state")
    is_complete: bool = Field(default=False, description="Whether execution has completed")
    program_output: str = Field(default="", description="Program stdout output from MARS execution")
    instruction_analysis: Optional[InstructionAnalysis] = Field(default=None, description="MIPS-computed instruction analysis")
...
class ExecuteRequest(BaseModel):
    ...
    mode: str = Field(default="step", pattern="^(full|step)$", description="Execution mode: 'full' or 'step'")


class ExecuteResponse(BaseModel):
    """Response from execution endpoints."""
    success: bool = Field(description="Whether the operation succeeded")
    state: Optional[ExecutionState] = Field(default=None, description="Current execution state")
...
class StepResponse(BaseModel):
    ...
    state: Optional[ExecutionState] = Field(default=None, description="Current execution state after step")
...
class AllocateResponse(BaseModel):
    ...
    address: Optional[int] = Field(default=None, description="Address of allocated block")
    heap: Optional[HeapState] = Field(default=None, description="Updated heap state")
    error: Optional[str] = Field(default=None, description="Error message if allocation failed")
...
class FreeResponse(BaseModel):
    ...
    heap: Optional[HeapState] = Field(default=None, description="Updated heap state")
    error: Optional[str] = Field(default=None, description="Error message if free failed")
...
class ResetResponse(BaseModel):
    ...
    state: Optional[ExecutionState] = Field(default=None, description="Initial execution state after reset")
    error: Optional[str] = Field(default=None, description="Error message if reset failed")
